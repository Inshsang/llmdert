...
from utils.utils import (classification_acc,parse_bbox_3d_Vis,cal_aro_3d)
import wandb,tqdm
...
def make_args_parser():
    ...
    parser.add_argument("--warm_lr", default=4e-6, type=float)
    ...
    ### Decoder
    parser.add_argument("--dec_nlayers", default=9,
                        type=int)  # note: the light FFN which is regarded as a simple decoder layer
    ...
    parser.add_argument("--repeat_num", default=5,
                        type=int)  # if you want use the model without NMS, please set the repeat_num==0
    ...
    parser.add_argument("--angle_type", default="", type=str, choices=['world_coords', 'object_coords'],
                        help="Specify the type of angle in 'world coordinate system' or 'object coordinate system', which is no difference in Scannet dataset.")
    ...
    ##### Dataset #####
    parser.add_argument(
        "--dataset_name", type=str, choices=["mynet", "sunrgbd", "scannet"], default="mynet"
    )
    parser.add_argument(
        "--dataset_root_dir",
        type=str,
        default='/media/kou/Data1/htc/Chat-3D-v2/V-DETR/data',
...
#################################################################################################
    ...
    parser.add_argument("--auto_test", default=True, action="store_true")

    parser.add_argument("--test_no_nms", default=False, action="store_true",
                        help="if you want use the model without NMS, please set the repeat_num==0")
    ...
    parser.add_argument("--test_ckpt", default='/media/kou/Data1/htc/Chat-3D-v2/V-DETR/ckpt/mymodel.pth/checkpoint42.pth',
                        type=str)
    ...
    #                     type=str)
    parser.add_argument("--checkpoint_dir", default='/media/kou/Data1/htc/Chat-3D-v2/V-DETR/ckpt/mymodel.pth', type=str)
    ...
    parser.add_argument("--wandb_key", default="81ba849aa88a5d6550ae793176e1c03174ee46f9", type=str)
...
def auto_reload(args):
    ignore_keys = [
        "ngpus","batchsize_per_gpu","eval_every_epoch","num_points",
        ...
        "no_per_class_proposal", "use_cls_confidence_only", "test_size",
         "dist_url", "model_name", "dataset_root_dir", "meta_data_dir", "checkpoint_dir", "dataset_name"
    ]
...
def do_train(
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        use_evaluate = ((epoch != 0) and (epoch % args.eval_every_epoch == 0 or epoch == (args.max_epoch - 1))) or (
                    epoch == 10)
        ...
        if is_primary():
            ...
            if args.wandb_activate:
                print('epoch:',str(epoch),';',log_message)
                # wandb_log(
                #     data=log_message,
                #     step=wandb_iter,
        ...
        if use_evaluate:
            ...
            if is_primary():
                ...
                log_message = dict( \
                    ...
                    val_loss_giou=wandb_val_loss_details[
                        'loss_giou'].item() if 'loss_giou' in wandb_val_loss_details else 0.0, )
