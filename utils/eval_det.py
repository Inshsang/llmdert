...
def eval_det_multiprocessing(
    ...
    for classname in gt.keys():
        if classname in pred:
            ret_values.append(
                eval_det_cls_wrapper((pred[classname], gt[classname], ovthresh, use_07_metric, get_iou_func, size)))

    # ret_values 现在包含了所有类别的评估结果
            import datetime
            # 获取当前时间
