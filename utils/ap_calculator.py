...
def parse_predictions(
        predicted_boxes, sem_cls_probs, objectness_probs, angle_probs, point_cloud, config_dict,
        predicted_boxes_CSA=None
):
    ...
    if config_dict["remove_empty_box"]:
        ...
        predicted_boxes_CSA[..., :, 2] = predicted_boxes_CSA[..., :, 2] - predicted_boxes_CSA[..., :,
                                                                          5] / 2  # bottom center
        ...
        inbox_inds = points_in_boxes_all(new_xyz, predicted_boxes_CSA).permute(0, 2,
                                                                               1).contiguous()  # bs,nboxes,npoints
    ...
    if config_dict["rotated_nms"]:
        ...
        for i in range(bsize):
            ...
            for j in range(K):
                ...
                boxes_3d_with_prob[j, 6] = obj_prob[i, j] * angle_probs[i, j] if config_dict["angle_nms"] else obj_prob[
                    i, j]
...
class APCalculator(object):
    ...
    def step_meter(self, outputs, targets):
        ...
        predicted_box_CSA = torch.cat((predicted_box_center, predicted_box_size, predicted_box_angle.unsqueeze(-1)),
                                      dim=-1)
