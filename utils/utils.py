...
def cal_in_3d(bbox1, bbox2):
    '''
        box [x1, y1, z1, l, w, h]
    '''
    bbox1[3:] = [max(0.01,i) for i in bbox1[3:]]
    bbox2[3:] = [max(0.01, i) for i in bbox2[3:]]
    multi = 1

    x, y, z = bbox2[:3]
    x1, y1, z1, l, w, h = bbox1

    if x1 - abs(l / 2) * multi <= x <= x1 + abs(l / 2) * multi and y1 - abs(w / 2) * multi <= y <= y1 + abs(
            w / 2) * multi and z1 - abs(h / 2) * multi <= z <= z1 + abs(h / 2) * multi:
        return 1
    else:
        return 0

def cal_aro_3d(bbox1, bbox2):
    '''
        box [x1, y1, z1, l, w, h]
    '''
    bbox1 = [max(0.01,i) for i in bbox1]
    bbox2 = [max(0.01, i) for i in bbox2]
    multi = 1
...
def cal_path_3d(S,L):
    ...
    for l_point in L:
        min_distance = float('inf')  # 初始设置为正无穷大，确保第一次计算的距离会被记录
        for s_point in S:
            distance = math.sqrt((l_point[0] - s_point[0])**2 + (l_point[1] - s_point[1])**2)
...
def parse_bbox_3d_Nav(text):
    ...
    for i in range(0, len(num_list), 2):
        cur_bbox = [num_list[j] for j in range(i, i + 2)]
...
if __name__ == '__main__':
    ...
    L = [(0, 0), (2, 2), (1, 1)]
    S = [(0, 0),(1, 1)]
    result = cal_path_3d(S, L)
    print("每个L端点到S上最近点的距离总和:", result)