...
class ScannetDatasetConfig(object):
    def __init__(self):
        ...
        # ])
        self.mean_size_arr = np.array([#与数据集yz不一致
            [0.76966726, 0.81160211, 0.92573741],
            [1.876858  , 1.84255952, 1.19315654],
            [0.61327999, 0.61486087, 0.71827014],
            [1.39550063, 1.51215451, 0.83443565],
...
class ScannetDetectionDataset(Dataset):
    def __init__(
        ...
        self.data_path = "/media/kou/Data3/htc/scannet_train_detection_data/"
        all_scan_names = list(
            set(
                [
    ...
    def __getitem__(self, idx):
        ...
        target_bboxes = np.zeros((MAX_NUM_OBJ, 6), dtype=np.float32)  # 存储目标边界框
        target_bboxes_mask = np.zeros((MAX_NUM_OBJ), dtype=np.float32)  # 存储目标边界框的掩码
        angle_classes = np.zeros((MAX_NUM_OBJ,), dtype=np.int64)  # 存储角度类别
        angle_residuals = np.zeros((MAX_NUM_OBJ,), dtype=np.float32)  # 存储角度残差
        size_residuals = np.zeros((MAX_NUM_OBJ, 3), dtype=np.float32)  # 存储尺寸残差
        raw_sizes = np.zeros((MAX_NUM_OBJ, 3), dtype=np.float32)  # 存储原始尺寸
        raw_angles = np.zeros((MAX_NUM_OBJ,), dtype=np.float32)  # 存储原始角度
        ...
        target_bboxes[0: instance_bboxes.shape[0], :] = instance_bboxes[:, [0,2,1,3,5,4]]
        ...
        # from utils.showbbox import showbbox
        # mesh_vertices = np.load(os.path.join(self.data_path, scan_name) + "_vert.npy")
        # point_clouds = mesh_vertices
        # point_clouds[:, 3:] /= 255
        # change = box_corners[:,:, [0, 2, 1]]
        # change[:,:,2] = -1*change[:,:,2]
        # showbbox(np.asarray(change),point_clouds)
        # pcd = o3d.geometry.PointCloud()
        # pcd.points = o3d.utility.Vector3dVector(point_clouds[:, :3])
        # pcd.colors = o3d.utility.Vector3dVector(point_clouds[:, 3:])
        # o3d.io.write_point_cloud('/media/kou/Data1/htc/Chat-3D-v2/V-DETR/test.ply',pcd)
...
class Bench(ScannetDetectionDataset):
    def __init__(
            self,
            dataset_config,
            split_set="train",
        ...
        super(Bench, self).__init__(
            ...
            augment=augment,
            use_random_cuboid=use_random_cuboid,
            random_cuboid_min_points=random_cuboid_min_points,
            args=args,)
        self.data_path = '/media/kou/Data3/htc/scene'
        self.class_mapping = {
            "cabinet": 0,
            "bed": 1,
            "chair": 2,
        ...
        # to_remove = [207, 278, 772, 808, 986, 1203, 1333, 1407, 1662, 2829, 3077, 3933, 4108, 4587, 4621, 6062, 6301, 6559, 6614, 6728, 7498, 8374, 8750, 9294, 9462, 9532, 9716]
        # all_scan_names = [name for name in all_scan_names if name not in to_remove]
        all_scan_names.sort()
        if split_set == "train":
            self.scan_names = all_scan_names[500:10000]
        # if split_set == "val":
        else:
            self.scan_names = all_scan_names[0:1]
        gt = {}
    ...
    def __getitem__(self, idx):
        scan_name = str(self.scan_names[idx])
        mesh_vertices = o3d.io.read_point_cloud('/media/kou/Data3/htc/scene/' + scan_name + ".ply")

        # mesh_vertices = o3d.io.read_point_cloud('/media/kou/Data1/htc/Chat-3D-v2/V-DETR/myscene/8.ply')
        points = np.asarray(mesh_vertices.points)  # 获取点云的坐标信息
        colors = np.asarray(mesh_vertices.colors)  # 获取点云的颜色信息
        mesh_vertices = np.concatenate((points, colors), axis=1)
        # point_clouds = np.concatenate((points, colors), axis=1)

        # point_cloud[:, 3:] = point_cloud[:, 3:] - np.array([[0.5]])
        ...
        if not self.use_color:
            point_cloud = points # do not use color for now
            pcl_color = colors*255
        else:
            point_cloud = mesh_vertices[:, 0:6]
        ...
        instance_bboxes = process_gt(self.gt[scan_name],self.class_mapping)
        ...
        instance_bboxes = instance_bboxes[:80,:]
        target_bboxes_mask[0 : instance_bboxes.shape[0]] = 1
        target_bboxes[0 : instance_bboxes.shape[0], :] = instance_bboxes[:, 0:6]
        ...
        size_residuals[0:instance_bboxes.shape[0], :] = \
            raw_sizes[0:instance_bboxes.shape[0], :] - self.dataset_config.mean_size_arr[target_bboxes_semcls[0: instance_bboxes.shape[0]], :]

        ret_dict["gt_box_sem_cls_label"] = target_bboxes_semcls.astype(np.int64)
        ret_dict["gt_box_present"] = target_bboxes_mask.astype(np.float32)
        ret_dict["scan_idx"] = np.array(scan_name).astype(np.int64)
        ret_dict["pcl_color"] = pcl_color
        ret_dict["point_clouds"] = torch.from_numpy(point_cloud.astype(np.float32))
        ret_dict["gt_box_sizes"] = raw_sizes.astype(np.float32)
        ret_dict["gt_box_sizes_normalized"] = box_sizes_normalized.astype(np.float32)
        ...
        # from utils.showbbox import showbbox
        # point_clouds = np.concatenate((point_cloud,pcl_color/255),axis=1)
        # # point_clouds[:, 3:] /= 255
        # change = box_corners[:,:, [0, 2, 1]]
        # change[:,:,2] = -1*change[:,:,2]
        # showbbox(np.asarray(change),point_clouds)
